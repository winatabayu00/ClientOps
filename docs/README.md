...
| Notifications and worker | P1 | M7 | Notification triggers | Jobs/notifications/outbox | Notification endpoints | Notification states | Assignment and release publication enqueue transactional outbox events. The worker atomically claims events, creates idempotent in-app notifications, optionally sends SMTP email, and retries failures after five minutes. Scheduled SLA/follow-up polling remains idempotent. |
