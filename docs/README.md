...
| Notifications and worker | P1 | M7 | Notification triggers | Jobs/notifications | Notification endpoints | Notification states | Transactional in-app notifications for release publish and issue assignment; idempotent 5-minute PostgreSQL polling for SLA approaching/follow-up overdue. Redis is provisioned but no queue client exists, so queue retries/dead-letter handling are deliberately not implemented. Failed polling runs log and retry next interval. |
