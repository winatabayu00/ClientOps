...
function Login() {
  const nav = useNavigate(); const [error, setError] = useState(''); const mutation = useMutation({ mutationFn: ({ email, password }: { email: string; password: string }) => auth.login(email, password), onSuccess: () => nav('/clients') })
...
function Shell() {
  ...
  return <><header><strong>ClientOps</strong><nav aria-label="Main navigation">{['clients', 'issues', 'releases', 'handoffs', 'follow-ups'].map(x => <NavLink key={x} to={`/${x}`}>{x.replace('-', ' ')}</NavLink>)}</nav><span>{me.data.name}</span><button onClick={() => logout.mutate()} disabled={logout.isPending}>Sign out</button></header><main><Outlet context={me.data} /></main></>
}
...
export default function App() { return <QueryClientProvider client={client}><Routes><Route path="/login" element={<Login />} /><Route element={<Shell />}><Route path="/clients" element={<Clients />} /><Route path="/issues" element={<Issues />} /><Route path="/issues/:id" element={<IssueDetail />} /><Route path="/releases" element={<Releases />} /><Route path="/handoffs" element={<Operations />} /><Route path="/follow-ups" element={<Operations followUps />} /></Route><Route path="*" element={<Navigate to="/clients" replace />} /></Routes></QueryClientProvider> }
